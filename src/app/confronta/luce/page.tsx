...
export default function ConfrontaLucePage() {
  ...
  const calcolaRisparmio = () => {
    ...
    const lista = metodoFiltro === 'TUTTI'
      ...
      const nuovaBollettaTotale = costiFissiStimati + materiaEnergiaNuova + pcvOfferta;
    ...
    const righeTop3 = top3
      .map((o, i) => `${i + 1}. ${o.nome} (${o.gestore}) · ${o.prezzo}€/kWh · PCV ${o.costo_fisso}€ · ${o.durata > 0 ? `blocco ${o.durata} mesi` : 'indicizzato'} → +${o.risparmio.toFixed(0)}€/anno`)
      .join('%0A');
    const msg = top3.length > 0
      ? `Ciao Bruno, ho fatto il confronto Luce su pogio.it.%0AConsumo: ${consumoNum} kWh · Spesa attuale: ${spesaNum}€.%0ALe 3 migliori sulla mia stima:%0A${righeTop3}%0AVoglio l'ANALISI DETTAGLIATA gratuita: ti giro subito la foto della bolletta.`
      : `Ciao Bruno, ho fatto il confronto Luce su pogio.it.%0AConsumo: ${consumoNum} kWh · Spesa attuale: ${spesaNum}€.%0ANon risultano offerte vantaggiose: voglio comunque l'ANALISI DETTAGLIATA gratuita, ti giro la foto della bolletta.`;
                ...
                <div key={offerta.id} className={`bg-white rounded-xl shadow-sm p-6 ${index === 0 ? 'ring-2 ring-green-500' : ''}`}>
                  ...
                  <div className="mt-4 bg-slate-50 border border-slate-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-center mb-3">
                      <div>
                        <p className="text-[11px] text-slate-500">Materia energia</p>
                        <p className="text-sm font-bold text-slate-800">{offerta.prezzo}€/kWh</p>
                      </div>
                      <div>
